...
export default async function getPoolSnapshot(
  ...
  const roundStartValue =
    roundStarted.args.totalTokenSupply * roundStarted.args.tokenValue;
  const stats = {
    ...EMPTY_STATS,
  ...
  const tokensBurnableForRound =
    latestWithdrawEvent?.args.tokensBurnableForRound ?? 0;
  ...
  positionOpenedEvents.forEach((x) => {
    if (x.block >= roundStartBlock && x.timestamp <= endTimestamp) {
      ...
      const isLong =
        x.args.tradeType == TradeType.LONG_CALL ||
        x.args.tradeType == TradeType.LONG_PUT;
      if (
        x.args.tradeType == TradeType.LONG_CALL ||
        x.args.tradeType == TradeType.SHORT_CALL
      ) {
        perListingExposure[x.args.listingId].callExposure +=
          x.args.amount * (isLong ? -1 : 1);
      } else {
        perListingExposure[x.args.listingId].putExposure +=
          x.args.amount * (isLong ? -1 : 1);
      }
    }
  });
  ...
  for (const listingId of Object.keys(perListingExposure)) {
    ...
    let lastCacheUpdate = nullthrows(
      listingCacheUpdatedEvents.find(
        (x: any) => x.block >= roundStartBlock && x.timestamp <= endTimestamp
      )
    );
    ...
    stats.openInterestValue +=
      perListingExposure[listingId].callExposure * callPrice +
      perListingExposure[listingId].putExposure * putPrice;
  }
  ...
  stats.quoteValue =
    stats.totalValue - stats.openInterestValue - stats.baseValue;
...
export async function syncPoolStats(deployment: Deployments = "mainnet-ovm") {
  ...
  const currentTime = (await getNetworkProvider(deployment).getBlock("latest"))
    .timestamp;

  for (const market of ["sETH", "sBTC", "sLINK"]) {
    const roundStartedEvents = (
      await getEventsFromLyraContract(
        deployment,
        "LiquidityPool",
        "RoundStarted",
        market
    ...
    const signalWithdrawEvents = (
      ...
      return {
        ...
        args: {
          ...
          tokensBurnableForRound:
            parseInt(x.args.tokensBurnableForRound) / 1e18,
        },
      };
    });
      ...
      return {
        ...
        args: {
          ...
          totalQuoteAmountReserved:
            parseInt(x.args.totalQuoteAmountReserved) / 1e18,
        },
      };
    });
    ...
    const boardCreatedEvents = await getEventsFromLyraContract(
      deployment,
      "OptionMarket",
      "BoardCreated",
      market
    ...
    ).forEach((listingAddedEvent) => {
      const { boardId, listingId, strike } = listingAddedEvent.args;
      const boardCreatedEvent = nullthrows(
        boardCreatedEvents.find((event) => event.args.boardId === boardId)
      );
    ...
    const roundStartedSortedEvents = roundStartedEvents.sort(
    ...
    for (const roundStarted of roundStartedSortedEvents) {
      ...
      while (true) {
        ...
        if (
          ...
          snapshotTimestamp >= maxExpiryTimestamp
        ) {
          break;
        }

        snapshotTimestamp = Math.min(
          snapshotTimestamp + SNAPSHOT_LENGTH,
          currentTime,
          maxExpiryTimestamp
        );
      }
  ...
  console.log(
    "-",
    "Update",
    Collections.PoolStatsSnapshots,
    documents.length,
    "items"
  );
