...
> {
  ...
  const roundEndedEvents = await getEventsFromLyraContract(
    params.deployment,
    "LiquidityPool",
    "RoundEnded",
    market
  ...
  const transferEvents = await getEventsFromLyraContract(
    params.deployment,
    "LiquidityCertificate",
    "Transfer",
    market
  ...
  const roundStartedEvent = roundStartedEvents.find((x) => {
    return (
      (x.args.newMaxExpiryTimestamp || x.args.newMaxExpiryTimestmp) ==
      params.maxExpiryTimestamp
    );
  });
  if (!roundStartedEvent) {
    console.log(
      `- WARNING: round with expiryTimestamp ${params.maxExpiryTimestamp} has not started`
    );
    return {};
  }
  ...
  for (const dataModifiedEvent of dataModifiedEvents) {
    if (dataModifiedEvent.block <= roundStartedEvent.block) {
      certificates[dataModifiedEvent.args.certificateId] = {
        value:
          ((dataModifiedEvent.args.liquidity /
            expiryToTokenValue[dataModifiedEvent.args.enteredAt]) *
            expiryToTokenValue[
              roundStartedEvent.args.lastMaxExpiryTimestamp ||
                roundStartedEvent.args.lastMaxExpiryTimestmp
            ]) /
          1e18,
        owner: "",
  ...
  const totalLiquidity = Object.values(liquidityPerOwner).reduce(
