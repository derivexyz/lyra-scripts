...
import { Collections, FirebaseCollections } from '../constants/collections'
...
import { AccountRewardEpoch, GlobalRewardEpoch, getEmptyAccountEpoch } from '../utils/avalonRewardEpoch';
...
export default async function syncAvalonRewards() {
  ...
  if (DROP_DB) {
    ...
    const globalRewardsCollectionReference = db.ref(FirebaseCollections.AvalonGlobalRewardsEpoch)
    const accountRewardsCollectionReference = db.ref(FirebaseCollections.AvalonAccountRewardsEpoch)
    await Promise.all([
      globalRewardsCollectionReference.update({}),
      accountRewardsCollectionReference.update({})
    ])
  }
...
initializeFirebaseApp()
  ...
  .then(async () => {
    console.log("Syncing to Firebase complete!")
    await closeFirebaseDB()
    process.exit(0)
  })
  .catch((e: Error) => {
    console.log(e.stack);
